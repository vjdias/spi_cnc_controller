...
module spi_master_slave_integration_tb;
  ...
  // envia um frame LED_CTRL via MOSI e coleta 7 bytes de resposta via MISO
  task automatic spi_led_roundtrip(
    ...
    // coleta 7 bytes de resposta (em outra transação)
    spi_end();
    // Aguarda até que a pilha de TX publique a resposta na FIFO
    wait_cycles = 0;
  ...
  initial begin
    ...
    `TEST_ASSERT(dec.frameIdEcho == 8'hA1,     "frameid")
    `TEST_ASSERT(dec.ledMask == 8'h07,         "mask")
    `TEST_ASSERT(dec.status == 8'h00,          "status")
    `TEST_ASSERT(led_control_response_pkg::check_parity(dec), "parity")
