...
package spi_service_pkg;
  ...
  localparam int REQ_MAX_BYTES  = move_queue_add_request_pkg::FRAME_BITS / 8;  // 42
  localparam int RESP_MAX_BYTES = move_probe_level_response_pkg::FRAME_BITS / 8; // 20

  // Profundidade da FIFO de recepção: 5 vezes o maior frame de requisição
  localparam int RX_FIFO_DEPTH   = REQ_MAX_BYTES * 5;   // 210 bytes
  // Nível de bloqueio: acima de 4 frames completos ocupados
  localparam int RX_BLOCK_LEVEL  = REQ_MAX_BYTES * 4;   // 168 bytes

  // Profundidade da fila de transmissão: 10 vezes o maior frame de resposta
  localparam int TX_FIFO_DEPTH   = RESP_MAX_BYTES * 10; // 200 bytes
endpackage
`endif
