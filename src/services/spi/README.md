...
## Pipeline de RX
- `spi_rx_slave_service`: adapta o wrapper `spi_slave` (registros `rd_en/raddr/rdata/irq`) para um fluxo de bytes (`spi_byte_valid/spi_byte`).
...
## Dicas de depuração
- Monitore: `u_cons.frame_valid`, `u_cons.frame_error`, `u_cons.out_msgType`, `u_cap.fifo.count`, `irq`, `spi_byte`, `overflow_error`.
